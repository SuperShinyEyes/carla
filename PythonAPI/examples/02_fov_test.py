...
def get_camera(world, parent_actor, height, width, gamma, yaw, pitch, fov):
...
class Recorder(object):
    ...
    def _reset_camera(self, yaw: float, pitch: float, fov: float):
    ...
    def reset(self, yaw: float, pitch: float, fov: float):
        self._reset_car()
        self._reset_camera(yaw, pitch, fov)
    ...
    def record(
        ...
        with CarlaSyncMode(
            self.world, self.camera, fps=self.fps
        ) as sync_mode:
...
def _make_dataset_directory() -> Path:
    path = "camera_fov_dataset-{0:%Y_%m_%d-%H_%M_%S}".format(datetime.now())
    path = Path(path)
    path.mkdir()
    return path
...
def main(
    ...
    video_rgb_name_template = "{index:03d}-fov={fov}.mp4"
    ...
    for i, fov in enumerate(range(10, 181, 1)):
        pitch: float = 0 #get_random_angle()
        yaw: float = 0 #get_random_angle()
        video_rgb_name = video_rgb_name_template.format(index=i, fov=fov)
        ...
        try:
            recorder.record(
                video_rgb_path, yaw=yaw, pitch=pitch, fov=fov
            )
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(description="Car FOV dataset generator")
